...
const UploadingExcel = ({handleFileChange, handleUpload, file,formRef , content , title}) => {
  ...
  return (
    <div className='bg-white w-full flex flex-col p-6 py-4 rounded-sm shadow-md'>
        ...
        <TbBrandTelegram className='text-teal-500 mx-1' size={20}/>
            <span className='text-lg font-medium uppercase text-teal-500'>{title}</span>
            </div>
        ...
        <div className='w-full border-t border-gray-200'>
            ...
            <div className='border p-2 px-4 border-teal-100 bg-teal-50 shadow-md mt-4 rounded-sm w-full'>
                ...
                {/* [Incident Id, Severity, Subject, Location, Region, User Name, System ID, Engineer Name, Status, Resolution Date] */}
                 {content}
                </span>
            </div>
            ...
            <button onClick={()=>handleUpload()} className='uppercase border border-teal-500 hover:bg-transparent hover:text-teal-500 delay-100 duration-100 transition-all font-semibold px-4 py-1.5 shadow-md rounded-sm bg-teal-500 text-white'>upload</button>
                {/* <a href={('/assets/sample-sheet.xlsx')} download={"sample-sheet.xlsx"} >
                <button className='bg-teal-500 rounded-sm shadow-md text-white hover:bg-transparent hover:text-teal-500 border border-teal-500 px-3 py-1 delay-150 duration-150 transition-all flex justify-center items-center'>
                    <FaDownload size={20} className='mx-1'/>
                Download Sample
